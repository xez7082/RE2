...
class ResidentEvilCard extends LitElement {
      ...
      <div style="${sizeStyle}background:#050005;border:1px solid ${cR}22;border-radius:8px;
        ...
        <div style="background:#0d0008;border-bottom:2px solid ${cR}33;padding:10px 16px;
          ...
          <div style="padding:8px 14px;background:#0a0015;border:1px solid ${cB}44;border-radius:6px;display:flex;gap:14px;align-items:center;">
            ...
            ${corpVal?html`
            <div style="border-left:1px solid ${cB}33;padding-left:14px;">
              <div style="font-size:10px;color:${cB};opacity:0.7;margin-bottom:3px;letter-spacing:1px;">CORPULENCE</div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;">
          ...
          <!-- 💪 COMPOSITION — col 1 -->
          <div style="padding:10px 14px;border-right:1px solid ${cR}15;">
          ...
          <!-- 🌙 SOMMEIL — pleine largeur si données présentes -->
          ${byCat('sommeil').length>0?html`
          <div style="grid-column:1/-1;padding:10px 14px;border-top:1px solid ${cR}15;">
  ...
  _renderAtelierWidget(w, sizeStyle, noBorder=false) {
    ...
    const machines = (w.machines||[]).map(m => {
      const pw  = fv(m.power_entity);
      const cur = fv(m.current_entity);
      // Si pas de power_entity, utiliser V*A
      const computedW = (pw==null && fv(m.voltage_entity)!=null && cur!=null)
                        ? fv(m.voltage_entity) * cur : null;
      const power = pw ?? computedW;
      return { name:m.name||'Machine', power, voltage:fv(m.voltage_entity),
               current:cur, day:fv(m.day_entity), color:m.color||'#f59e0b' };
    });
    ...
    const nbActive   = machines.filter(m=>m.power!=null&&m.power>thresh||m.current!=null&&m.current>0.1).length;
    ...
    return html`
      <div style="${sizeStyle}background:#0a0600;border:2px solid ${cD}55;border-radius:8px;
                  overflow:hidden;font-family:'Courier New',monospace;">
        <style>
          @keyframes _at2_scan{0%{left:-40%}100%{left:110%}}
          @keyframes _at2_pulse{0%,100%{opacity:1}50%{opacity:0.3}}
          @keyframes _at2_blink{0%,100%{opacity:1}50%{opacity:0}}
        </style>

        <!-- HEADER -->
        <div style="background:#140900;border-bottom:2px solid ${cD}55;padding:10px 16px;
                    display:flex;align-items:center;justify-content:space-between;position:relative;overflow:hidden;">
          <div style="position:absolute;top:0;left:0;right:0;height:2px;overflow:hidden;">
            <div style="position:absolute;width:40%;height:2px;background:${cD};animation:_at2_scan 4s linear infinite;"></div>
          </div>
          <div style="font-size:15px;color:${cD};font-weight:900;letter-spacing:2px;">
            ${w.title||'⚡ SÉCURITÉ ÉLECTRIQUE — ATELIER'}
          </div>
          <div style="font-size:13px;font-weight:900;padding:5px 14px;border-radius:4px;
        ...
        <!-- BARRE CHARGE -->
        <div style="padding:8px 16px;border-bottom:1px solid ${cA}22;background:#0c0500;">
          ...
          <div style="height:8px;background:#1a0800;border-radius:4px;overflow:hidden;border:1px solid ${cA}22;">
            <div style="height:100%;width:${Math.max(loadPct,0.5)}%;background:linear-gradient(90deg,${cG},${cA},${loadCol});border-radius:4px;transition:width .8s;"></div>
          </div>
        </div>

        <!-- MACHINES EN 2 COLONNES -->
        ...
        <div style="padding:6px 16px;display:flex;justify-content:space-between;font-size:12px;
                    color:${cA}88;border-top:1px solid ${cA}22;margin-top:4px;">
          <span>CIRCUIT ${w.circuit_label||'ATELIER'} · ${w.circuit_voltage||230}V / ${circuitMax}A</span>
          <span style="animation:_at2_blink 1.2s step-end infinite;color:${cG};">● LIVE</span>
        </div>
      </div>`;
  }
  ...
  // ════════════════════════════════════════════════════════════════
  //  CARTE ROBOT ZOOMABLE — image (carte Dreame/MOVA) avec pinch-zoom,
  //  pan tactile (un doigt) et boutons +/−/reset. Pas de géoréférencement
  ...
  _renderZoomMap(camUrl, key, opts = {}) {
    ...
    const h    = opts.height || 230;
    const live = !!opts.live;
    const lc   = opts.liveColor || '#22c55e';
    const btn  = 'width:26px;height:26px;border:none;border-radius:5px;background:rgba(20,20,30,.8);'
               + 'color:#e2e8f0;font-size:15px;font-weight:700;cursor:pointer;display:flex;'
               + 'align-items:center;justify-content:center;font-family:inherit;line-height:1;'
               + 'box-shadow:0 1px 4px rgba(0,0,0,.5);';
  ...
  _initZoomPan(container) {
    ...
    const onMove = (e) => {
      ...
      if (pointers.size === 2) {
        ...
        if (pinchDist > 0) { scale = pinchScale * (dist / pinchDist); clamp(); apply(); }
    ...
    const onUp = (e) => {
      ...
      if (pointers.size === 0) { dragging = false; container.style.cursor = 'grab'; }
    };
    const onWheel = (e) => {
      e.preventDefault();
      scale *= (e.deltaY < 0 ? 1.12 : 0.89); clamp(); apply();
    };
    const onDbl = (e) => { e.stopPropagation(); scale = 1; tx = 0; ty = 0; apply(); };
  ...
  static get WIDGET_SCHEMAS() {
    ...
    return {
      ...
      dreame: [
        ...
        {k:'map_height',l:'Hauteur carte (px)',t:N},
      ...
      gauge: [
        {k:'entity',l:'Entité principale',t:E},{k:'label',l:'Libellé',t:T},
        {k:'min',l:'Min',t:N},{k:'max',l:'Max',t:N},
        {k:'min_entity',l:'Min (entité)',t:E},{k:'max_entity',l:'Max (entité)',t:E},
        {k:'color',l:'Couleur',t:'color',d:'#00ff88'},
        {k:'unit',l:'Unité',t:T},{k:'decimals',l:'Décimales',t:N},
        {k:'fontSize',l:'Taille valeur (px)',t:N},{k:'labelSize',l:'Taille label (px)',t:N},
      ],
      sparkline: [
        {k:'entity',l:'Entité',t:E},{k:'label',l:'Libellé',t:T},
        {k:'color',l:'Couleur',t:'color',d:'#22d3ee'},
        {k:'hours',l:'Heures affichées',t:N},{k:'decimals',l:'Décimales',t:N},
        {k:'unit',l:'Unité',t:T},{k:'fontSize',l:'Taille valeur (px)',t:N},
      ],
      badge: [
        {k:'entity',l:'Entité principale',t:E},{k:'label',l:'Libellé',t:T},
        {k:'icon',l:'Icône (mdi:…)',t:T},{k:'color',l:'Couleur',t:'color',d:'#f59e0b'},
        {k:'decimals',l:'Décimales',t:N},{k:'unit',l:'Unité (override)',t:T},
        {k:'fontSize',l:'Taille valeur (px)',t:N},{k:'iconSize',l:'Taille icône (px)',t:N},
        {k:'iconPos',l:'Position icône',t:S,o:['left','top','right']},
        {k:'secondary_entity',l:'Date/heure (entité)',t:E},
        {k:'secondary_label',l:'Préfixe date (ex: "le")',t:T},
        {k:'tertiary_entity',l:'Texte additionnel (entité)',t:E},
        {k:'hud',l:'Cadre HUD (coins coupés)',t:'bool'},
      ],
      shape: [
        {k:'label',l:'Libellé',t:T},{k:'entity',l:'Entité (statut)',t:E},
        {k:'shape',l:'Forme',t:S,o:['circle','square','triangle','hexagon']},
        {k:'size',l:'Taille (px)',t:N},{k:'filled',l:'Remplie',t:'bool'},
        {k:'color',l:'Couleur',t:'color',d:'#ef4444'},
      ],
      progress: [
        {k:'entity',l:'Entité',t:E},{k:'label',l:'Libellé',t:T},
        {k:'min',l:'Min',t:N},{k:'max',l:'Max',t:N},
        {k:'unit',l:'Unité',t:T},{k:'decimals',l:'Décimales',t:N},
        {k:'color',l:'Couleur barre',t:'color',d:'#22c55e'},
        {k:'fontSize',l:'Taille valeur (px)',t:N},{k:'height',l:'Épaisseur barre (px)',t:N},
      ],
      button: [
        {k:'entity',l:'Entité',t:E},{k:'label',l:'Libellé',t:T},{k:'icon',l:'Icône (mdi:…)',t:T},
        {k:'action',l:'Action',t:S,o:['toggle','turn_on','turn_off','call_service']},
        {k:'show_state',l:'Afficher état',t:'bool'},{k:'style',l:'Style',t:S,o:['default','pill','icon']},
        {k:'color',l:'Couleur actif',t:'color',d:'#22c55e'},
        {k:'color_off',l:'Couleur inactif',t:'color',d:'#334155'},
        {k:'fontSize',l:'Taille texte (px)',t:N},{k:'iconSize',l:'Taille icône (px)',t:N},
      ],
      foundry: [
    ...
    const SECTIONS = {
      ...
      spa_temp:[
        {k:`spa_g${wi}`,t:'GENERAL — EAU & AIR',i:'🌡',c:'#00ccff',
         keys:['entity','targetEntity','extTempEntity','extHumEntity',
               'airTempEntity','airHumEntity','powerEntity','energyEntity']},
        ...
        {k:`spa_m${wi}`,t:'MAINTENANCE',i:'🔧',c:'#f59e0b',
        ...
        {k:`spa_cam${wi}`,t:'CAMERA & AFFICHAGE',i:'📷',c:'#94a3b8',
         keys:['cameraEntity','bgImage','color','view']},
      ],
      power_cell:[
        {k:`pc_g${wi}`,t:'CONFIGURATION',i:'🔋',c:'#00ccff',keys:['title']},
      ],
      plant:[
        {k:`pl_id${wi}`,t:'IDENTITE & PHOTO',i:'🌿',c:'#22c55e',
      ...
      tank:[
        {k:`tk_g${wi}`,t:'TITRE & CAPACITE',i:'💧',c:'#38bdf8',
        ...
        {k:`tk_e${wi}`,t:'EAU & PLUIE',i:'🌧',c:'#818cf8',keys:['inflow_entity','rain_entity']},
        {k:`tk_t${wi}`,t:'TEMPERATURES',i:'🌡',c:'#f59e0b',
      ...
      tracker:[
        {k:`tr_g${wi}`,t:'RADAR DE PRESENCE',i:'📡',c:'#818cf8',
      ...
      badge:[
        {k:`ba_v${wi}`,t:'VALEUR PRINCIPALE',i:'🌡',c:'#f59e0b',
         keys:['entity','label','icon','color','decimals','unit']},
        {k:`ba_s${wi}`,t:'DONNEES SECONDAIRES',i:'📅',c:'#818cf8',
         keys:['secondary_entity','secondary_label','tertiary_entity']},
        {k:`ba_d${wi}`,t:'AFFICHAGE',i:'🎨',c:'#94a3b8',
         keys:['fontSize','iconSize','iconPos','hud']},
      ],
      gauge:[
        {k:`ga_v${wi}`,t:'CAPTEUR & PLAGE',i:'📊',c:'#00ff88',
         keys:['entity','label','min','max','min_entity','max_entity']},
        {k:`ga_d${wi}`,t:'AFFICHAGE',i:'🎨',c:'#94a3b8',
         keys:['color','unit','decimals','fontSize','labelSize']},
      ],
      sparkline:[
        {k:`sp_v${wi}`,t:'CAPTEUR & STYLE',i:'📈',c:'#22d3ee',
         keys:['entity','label','color','hours','decimals','unit','fontSize']},
      ],
      progress:[
        {k:`pg_v${wi}`,t:'CAPTEUR & PLAGE',i:'📊',c:'#22c55e',
         keys:['entity','label','min','max','unit','decimals']},
        {k:`pg_d${wi}`,t:'STYLE',i:'🎨',c:'#94a3b8',
         keys:['color','fontSize','height']},
      ],
      button:[
        {k:`bt_v${wi}`,t:'ACTION',i:'🔘',c:'#22c55e',
         keys:['entity','label','icon','action','show_state','style']},
        {k:`bt_d${wi}`,t:'COULEURS & TAILLES',i:'🎨',c:'#94a3b8',
         keys:['color','color_off','fontSize','iconSize']},
      ],
      shape:[
        {k:`sh_v${wi}`,t:'FORME & STYLE',i:'⬡',c:'#ef4444',
         keys:['label','entity','shape','size','filled','color']},
      ],
      server:[
        {k:`sv_g${wi}`,t:'IDENTIFICATION',i:'🖥',c:'#22c55e',
         keys:['server_name','server_os','server_icon']},
        {k:`sv_s${wi}`,t:'CAPTEURS SYSTEME',i:'📊',c:'#38bdf8',
         keys:['cpu_entity','ram_entity','hdd_entity','status_entity',
               'uptime_entity','restart_entity','process_entity','gauge_size']},
      ],
      weather:[
        {k:`we_g${wi}`,t:'METEO PRINCIPALE',i:'🌤',c:'#38bdf8',
         keys:['animated','weather_config.weather','weather_config.uv',
               'weather_config.rain','weather_config.moon']},
        {k:`we_p${wi}`,t:'POLLENS & QUALITE AIR',i:'🌿',c:'#22c55e',
         keys:['weather_config.ng','weather_config.cg','weather_config.nb','weather_config.cb',
               'weather_config.na','weather_config.ca','weather_config.nu','weather_config.cu',
               'weather_config.nr','weather_config.crr','weather_config.no','weather_config.co',
               'weather_config.qp','weather_config.qa']},
      ],
      foundry:[
